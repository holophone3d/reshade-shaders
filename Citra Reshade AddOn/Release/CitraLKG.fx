...
{
	//buffer ratio is 400x240
	// half of the buffer = 400
    return tex2D(ReShade::BackBuffer, float2(normalized_coords.x/2.0, normalized_coords.y));
 //   return tex2D(RGBLeft, normalized_coords);
}

float4 getRightRGB(float2 normalized_coords)
{
    return tex2D(ReShade::BackBuffer, float2(0.5 + normalized_coords.x/2.0, normalized_coords.y));
  //return tex2D(RGBRight, normalized_coords);
}
...
float4 sampleVirtualQuiltImage(float2 normalized_coords, float alpha_range_offset, float focus_val)
{
...
float4 renderToLookingGlass(float2 normalized_coords, float alpha_range_offset, float focus_val)
{   
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        float4 col1 = sampleVirtualQuiltImage(texArr(coords1, tile, viewPortion), alpha_range_offset, focus_val);
        float4 col2 = sampleVirtualQuiltImage(texArr(coords2, tile, viewPortion), alpha_range_offset, focus_val);
...
float4 GenerateQuilt(float4 pos : SV_POSITION, float2 tex : TEXCOORD) : SV_TARGET {
    return sampleVirtualQuiltImage(tex, GetDepthiness(), GetFocus());
}

float4 GenerateLKG(float4 pos : SV_POSITION, float2 tex : TEXCOORD) : SV_TARGET {
    return renderToLookingGlass(tex, GetDepthiness(), GetFocus());
}
